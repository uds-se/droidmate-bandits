...
open class HybridEventProbabilityMF(modelName: String,
                                    arffName: String,
                                    useClassMembershipProbability: Boolean,
									private val psi: Double = 20.0) : EventProbabilityMF(modelName, arffName, useClassMembershipProbability) {

    constructor(modelName: String,
                arffName: String,
                useClassMembershipProbability: Boolean,
                modelPath: Path?,
				psi: Double = 20.0) : this(modelName, arffName, useClassMembershipProbability, psi) {
    ...
    *  (just so that we can add it into the output csv) */
	...
	override suspend fun dump(context: ExplorationContext) {
		...
		val file = context.getModel().config.baseDir.resolve("hybrid_model.csv")
		Files.write(file, out.lines())
	}

	override fun getProbabilities(state: StateData): Map<Widget, Double> {
		try {
			...
			return state.actionableWidgets
					.map { widget ->
						...
						val probabilities = getProbabilitiesUnsync(widget, state, widget.availableActions())
	...
	fun getProbabilitiesUnsync(widget: Widget, state: StateData, actionList: List<ExplorationAction>): Map<ExplorationAction, Double> {
		// Return the maximum event probability for a widget, given the types of event that can be triggered
	...
	fun getProbabilities(widget: Widget, state: StateData, actionList: List<ExplorationAction>): Map<ExplorationAction, Double> {
		try {
			runBlocking { mutex.lock() }
	...
	private fun addNewWidgets(newState: StateData) {
	...
	private fun Widget.getProbabilityFromModel(state : StateData): Double{
